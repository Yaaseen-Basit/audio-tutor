...
class SonificationEngine {
  ...
  // Short dual-tone chimes for orientation: start / end / anomaly / tempo change.
  playEarcon(kind) {
    if (!this.ctx || !this.unlocked) return;
    const patterns = {
      start: [660, 990],
    ...
    const [f1, f2] = patterns[kind] || [500, 500];
    const t0 = this.ctx.currentTime;
    [f1, f2].forEach((f, i) => {
      ...
      g.gain.linearRampToValueAtTime(0.32, start + 0.01);
...
- Limit the plan to AT MOST 6 nodes total so the response stays concise.
...
// Best-effort recovery for a JSON array of {"type":"function","name":...,
// "parameters":{...}} objects — another text shape some model backends emit.
function extractJsonFunctionCalls(text) {
  ...
  try {
    ...
    return arr
      ...
      .map((it) => ({ name: it.name, input: it.parameters || it.input || it.arguments || {} }));
  } catch (e) {
    return [];
  }
}

// Best-effort recovery for the <invoke name="..."><parameter name="...">value
// </parameter>...</invoke> text format some model backends emit instead of
// native tool_use blocks.
function extractXmlInvokeCalls(text) {
  const calls = [];
  ...
  while ((m = invokeRe.exec(text))) {
    const name = m[1];
    if (!["defineSpatialBounds", "setPlaybackTempo", "scheduleTone", "scheduleSpeechMarker"].includes(name)) continue;
    const input = {};
    let pm;
    paramRe.lastIndex = 0;
    while ((pm = paramRe.exec(m[2]))) {
      ...
      input[pm[1]] = raw !== "" && !isNaN(num) ? num : raw;
    }
...
export default function App() {
  ...
  const WELCOME_TEXT = "AudioTutor loaded. Press Space to play audio chart, Left or Right arrow keys to scrub points, or P to select presets.";

  useEffect(() => {
    const t = setTimeout(() => welcomeRef.current?.focus(), 60);
  ...
  // Any click, or Enter anywhere on the page, cleanly resumes the AudioContext
  // (a user gesture) instead of requiring the gate button specifically.
  const handleRootActivate = useCallback((e) => {
  ...
  async function runAgent() {
    ...
    try {
      ...
      textBlocks.forEach((b) => {
        if (/<invoke name=/.test(b.text) || /"type"\s*:\s*"function"/.test(b.text)) {
          log("system", "Model returned tool calls as inline text — parsing them directly.");
        } else {
          log("reasoning", b.text.trim());
        }
      });
      if (toolBlocks.length === 0 && textBlocks.length > 0) {
        const recovered = textBlocks.flatMap((b) => {
          const jsonCalls = extractJsonFunctionCalls(b.text);
          if (jsonCalls.length > 0) return jsonCalls;
          const xmlCalls = extractXmlInvokeCalls(b.text);
          if (xmlCalls.length > 0) return xmlCalls;
          return extractPseudoToolCalls(b.text);
        });
      ...
      if (pending) planNodes.push(pending);
      // Drop a trailing node with no speech label — it's almost always an
      // artifact of the response being cut off mid-plan, not real data.
      while (planNodes.length > 1 && !planNodes[planNodes.length - 1].textLabel) {
        planNodes.pop();
      }
  ...
  const clearPendingAudioTimers = useCallback(() => {
  ...
  const playNode = useCallback((idx) => {
    ...
    if (!node || !engineRef.current.unlocked) return 0;
    clearPendingAudioTimers();
    engineRef.current.stopSpeech();

    const isFirst = idx === 0;
    const isLast = idx === nodes.length - 1;
    const isAnomaly = node.timberType === "sawtooth";
    let cursor = 0;

    if (isFirst) { engineRef.current.playEarcon("start"); cursor += 200; }
    else if (isLast) { engineRef.current.playEarcon("end"); cursor += 200; }
    else if (isAnomaly) { engineRef.current.playEarcon("anomaly"); cursor += 200; }

    const toneTimer = setTimeout(() => engineRef.current.playTone(node), cursor);
    pendingTimersRef.current.push(toneTimer);

    const toneMs = Math.max(80, (node.durationSec || 0.4) * 1000);
    const label = node.textLabel || `Node ${idx + 1}`;
    const speechDelay = cursor + toneMs + 90; // let the tone finish before speech starts, avoids masking
    const speechTimer = setTimeout(() => engineRef.current.speak(label), speechDelay);
    pendingTimersRef.current.push(speechTimer);

    setLiveMsg(`Node ${idx + 1} of ${nodes.length}. ${label} Tone ${Math.round(quantizeFrequency(node.frequencyHz))} hertz.`);

    const estSpeechMs = Math.max(650, label.length * 55); // rough TTS duration estimate for pacing
    return speechDelay + estSpeechMs;
  }, [nodes, clearPendingAudioTimers]);

  const inspectCurrent = useCallback(() => {
    const node = nodes[currentIndex];
    if (!node) return;
    const label = node.textLabel || `Node ${currentIndex + 1}`;
    engineRef.current.stopSpeech();
    engineRef.current.speak(label);
    setLiveMsg(`Inspecting node ${currentIndex + 1} of ${nodes.length}. ${label}`);
  }, [nodes, currentIndex]);
  ...
  const cyclePreset = useCallback(() => {
    ...
    setPresetKey((prev) => {
      ...
      const msg = `Preset selected: ${PRESETS[next].label}`;
      engineRef.current.speak(msg);
      setLiveMsg(msg);
      return next;
    });
  }, []);

  const stepTo = useCallback((idx, e) => {
  ...
  useEffect(() => {
    ...
    if (currentIndex >= nodes.length) {
      setIsPlaying(false);
      setCurrentIndex(nodes.length - 1); // stay on the last real node, don't leave the index out of range
      return;
    }
    const totalMs = playNode(currentIndex);
    const extraGap = 150 / tempo; // small tempo-scaled buffer between nodes; speech itself stays at natural rate
    const gapMs = isFinite(totalMs) && totalMs > 0 ? Math.min(9000, Math.max(300, totalMs + extraGap)) : 600;
  ...
  const togglePlay = useCallback((e) => {
    ...
    if (!isPlaying && currentIndex >= nodes.length - 1) {
      setCurrentIndex(0); // sequence had finished — restart from the top
    }
    setIsPlaying((p) => !p);
  }, [nodes, isPlaying, currentIndex]);

  const adjustTempo = useCallback((delta) => {
    setTempo((t) => {
      const next = Math.max(0.25, Math.min(3, Math.round((t + delta) * 4) / 4));
      engineRef.current.playEarcon(delta > 0 ? "up" : "down");
      setLiveMsg(`Tempo ${next.toFixed(2)}x`);
      return next;
    });
  }, []);
  ...
  useEffect(() => {
    function onKey(e) {
      ...
      if (e.code === "Space" && tag === "BUTTON") return; // avoid double-firing with the button's own native activation
      if (e.repeat) return; // ignore OS key-repeat so holding a key doesn't spam toggles/steps
      ...
      else if (e.key === "[") { adjustTempo(-0.25); }
      else if (e.key === "]") { adjustTempo(0.25); }
      else if (e.key === "r" || e.key === "R") { if (currentIndex >= 0) playNode(currentIndex); }
      else if (e.key === "i" || e.key === "I") { inspectCurrent(); }
      else if (e.key === "p" || e.key === "P") { cyclePreset(); }
    }
    window.addEventListener("keydown", onKey);
    return () => window.removeEventListener("keydown", onKey);
  }, [togglePlay, stepTo, adjustTempo, currentIndex, nodes, playNode, inspectCurrent, cyclePreset]);
  ...
  return (
    <div className="at-root" onClick={handleRootActivate} onKeyDown={handleRootActivate}>
      <style>{`
        .at-root {
      ...
      <div ref={welcomeRef} tabIndex={-1} role="alert" aria-atomic="true" className="sr-only">
        {WELCOME_TEXT}
      </div>

      <div aria-live="assertive" aria-atomic="true" className="sr-only">{liveMsg}</div>
        ...
        <div className="at-grid">
          ...
          <div className="at-panel">
            ...
            <div className="at-canvas-wrap">
              <svg viewBox={`0 0 ${canvasW} ${canvasH}`} width="100%" height={canvasH} aria-hidden="true">
            ...
            <div className="at-transport">
              <button className="at-iconbtn" onClick={(e) => stepTo(currentIndex - 1, e)} disabled={nodes.length === 0} aria-label="Previous node"><SkipBack size={17} /></button>
              <button className="at-iconbtn" onClick={togglePlay} disabled={nodes.length === 0} aria-label={isPlaying ? "Pause" : "Play"}>
                {isPlaying ? <Pause size={18} /> : <Play size={18} />}
              </button>
              <button className="at-iconbtn" onClick={(e) => stepTo(currentIndex + 1, e)} disabled={nodes.length === 0} aria-label="Next node"><SkipForward size={17} /></button>
              <button className="at-iconbtn" onClick={(e) => { e.currentTarget.blur(); currentIndex >= 0 && playNode(currentIndex); }} disabled={currentIndex < 0} aria-label="Repeat current node"><RotateCcw size={16} /></button>
              <div className="at-tempo"><Activity size={13} aria-hidden="true" /> tempo ×{tempo.toFixed(2)}</div>
            </div>
            <div className="at-keys">
              <kbd>Space</kbd> play/pause · <kbd>←</kbd><kbd>→</kbd> step · <kbd>[</kbd><kbd>]</kbd> tempo · <kbd>R</kbd> repeat · <kbd>I</kbd> inspect · <kbd>P</kbd> cycle preset
            </div>
            ...
            <ul className="at-nodelist" aria-label="Sonification node sequence">
              {nodes.map((n, i) => (
                <li key={i} className={i === currentIndex ? "active" : ""} onClick={(e) => stepTo(i, e)}>
